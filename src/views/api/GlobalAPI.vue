<template>
  <main ref="globalApiRef" class="text-[14px]">
    ...
    <div id="mixins">
      ...
      <p>mixins在vue3中使用主要是为了向后兼容，更加推荐使用的是组合式函数，组合式函数是利用vue的组合式api来封装和复用状态逻辑的函数。</p>
      <p>在应用场景下，经常会复用公共任务的逻辑，可以在不同组件内共用这一逻辑，那么这个时候就可以使用组合式函数的行为。</p>
    ...
    <div id="provide">
      ...
      <p>provide 作为一个函数，其中第一个参数作为key，第二个参数作为值进行传入。在后代组件通过inject注入。</p>
      <pre>
        <code>
          // 父组件 Provide
    ...
    <div id="userid">
      ...
      <p>userId 生成每个id在每个应用内都是唯一的，它可以用做表单元素、无障碍生成的元素id。在同一组件内多次调用会生成不同的ID。</p>
      <pre>
        <code>
    ...
    <div id="directive">
      ...
      <p>directive 自定义指令；除了vue内置的一系列指令，比如v-model, v-if等，vue还允许你注册自定义指令。</p>
      <p>前面已经使用两中重用vue代码的方式，分别是组件和组件函数，组件在于页面构建的模块，组件函数侧重于有状态的逻辑，而自定义指令是为了重用涉及普通元素底层DOM的访问。</p>
      <p>一个自定义指令是一个包含组件生命周期的对象来定义，钩子函数会接受到来自指令所绑定的元素作为参数。</p>
      <p>如下一个自定义指令的例子，当vue元素被插入到DOM中后，该指令会将一个class插入到元素中：</p>
      <p v-highlight.v1.v2="isHighlight" class="p-[12px] bg-blue-400 rounded-[5px]">Directive：Hello Vue3</p>

      <p class="text-red-500">在 vue setup 中，任何以 v 开头的驼峰命令的变量都可以当做自定义指令使用。</p>
      <pre>
        <code>
          // Element node
